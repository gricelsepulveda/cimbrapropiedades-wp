...
<section class="cm-contact" id="contacto">
  <div class="flex row between cm-flex-contact">
    ...
    <?php
      if(isset($_GET['r'])){
        echo '<div id="sentMessage"><div>Mensaje enviado exitosamente<span><i class="fas fa-times"></i></span></div></div>';
      }
    ?>
    ...
    <form action="/mailer/mailer.php" class="flex col end cm-contact-form" method="post">
      ...
      <div class="flex row end cm-flex-form"><label>Mensaje:</label><textarea id="message" name="message" rows="7" type="text" required placeholder="Ingresa el mensaje"></textarea></div>
      <button class="button normal" type="submit">Enviar</button>
      <span class="cm-note">
      Te contactaremos de vuelta en un plazo max de 24hrs (hábiles)
      </span>
    </form>
  </div>
</section>